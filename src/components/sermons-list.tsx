  ...
  return (
    <>
      ...
      <div className="space-y-4">
        {isLoading ? (
          <div className="text-center py-8 text-muted-foreground">Loading sermons...</div>
        ) : filteredPosts.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            {searchQuery ? "No sermons found matching your search." : "No sermons found for the selected filter."}
          </div>
        ) : (
          filteredPosts.map((post) => (
            <SermonCard key={post.id} post={post} onOpen={() => setSelectedPostId(post.id)} />
          ))
        )}
      </div>
